...
| 167 | Menu Builder — hide Saturday/Sunday columns when no weekend meals (2026-08-06 deferred) — "Saturday — No meals added" / "Sunday — No meals added" is confusing for 5-day programs; fix: only render day columns that have items OR are Mon–Fri by default; Sat/Sun shown only if items exist for those days; alternatively add a "5-day / 7-day" toggle in the week header | ⏳ tomorrow |
...
### 🔴 HIGH PRIORITY — Kitchen/Vended Meal Billing (2026-08-22, not started)

Hashi flagged this while testing the kitchen side. Real gap: kitchens produce meals for sites but nothing in CACFPLink tracks what a sponsor owes a kitchen for that work. Deliberately NOT started yet — scoped as complex, revisit after kitchen-side testing is done.

**Requirements (Hashi's spec):**
- Contract is priced per meal type, not flat-rate.
- Kitchen is the entity being paid.
- Rates should be defined per meal type.
- Billing should be based on actual meals provided.
- Site should remain identifiable because a kitchen can serve multiple sites.
- Sponsor should be able to review/approve the bill.

**Research done (2026-08-22) — grounded in real CACFP vendor agreement templates (Wisconsin DPI, others), not assumption:**
- Real-world CACFP vendor contracts price per meal type (a $ rate for breakfast, a separate one for lunch/supper/snack), confirming rate-per-meal-type is the standard, not a flat monthly fee.
- The real invoice format is exactly: meal type × count × price, aggregated by period, with a total. This matches data CACFPLink already has — `delivery_plans`/`delivery_instances` and `production_records` already store per-meal-type counts per kitchen per day per site. Billing math could run on existing production/delivery data rather than needing a new meal-count source.
- **Document compliance should NOT gate billing.** Original assumption was wrong. Real vendor agreements withhold payment only for meal-quality/delivery violations at time of delivery (spoiled, doesn't meet CACFP meal pattern, delivered outside agreed time) — not for expired paperwork like insurance or W-9. Business compliance docs get vetted once, up front, during vendor selection — not re-checked per invoice. Keep Billing and Compliance as separate, unlinked features.
- Two distinct doc layers exist in real vendor agreements: (1) business compliance docs (W-9, insurance, health permit) — already what the Documents/Compliance page tracks; (2) per-delivery meal documentation (menus, production records, ingredient labels) — closer to what Production Records already captures. Billing doesn't need either as a gate.

**Not yet decided (deferred until Hashi is ready to scope the build):**
- Exact billing period (weekly vs monthly vs per-delivery)
- Whether rates are set once per kitchen or can vary per site the kitchen serves
- Invoice dispute/adjustment handling
- Whether this becomes a new "Kitchen Billing" nav item for sponsor + kitchen, or lives inside an existing page
...
Key tables: `organizations`, `users`, `applications`, `documents`, `notifications`, `meal_counts`, `routes`, `message_threads`, `messages`, `message_recipients`, `coordinator_assignments`, `delivery_plans`, `delivery_instances`

`kitchen_site_connections` table still exists in the DB but is unused as of 2026-08-26 — see Task #182 below. Leave any leftover rows alone; nothing reads them.
...
| 166 | Menu Builder bug fixes (2026-08-06) — (1) Clear Week button: DELETE /menus/:id/items/all endpoint + frontend "Clear Week" button (red, only visible when items exist, confirm dialog, reloads grid); (2) Menu import dropdown fix: VALID_COMPS_IM now uses 'milk'/'protein' instead of 'dairy'/'meat/alt' — milk items no longer show as "Grain" in review modal; (3) Extraction prompt fix: schema now explicitly lists "milk"\|"grain"\|"protein" so AI returns canonical names without needing aliases; (4) Menu access for sub-orgs: all listMenus/upsertItem/clearMenuItems/deleteItem/updateMenu/deleteMenu/generateMenu checks now use `sponsor_id = $1` pattern — sponsor can read/write menus for Joball Day and any kitchen/site they own; (5) False issue count: totalIssues now skips empty meal slots — Mon-Fri Complete but "58 issues" was phantom weekend validation | ✅ |
| 168 | Menu import overhaul (2026-08-08) — three root causes fixed: (1) **Wrong PDF model**: claude-haiku-4-5 does not support native PDF document blocks — all PDFs returned empty silently; switched to claude-3-5-sonnet-20241022 for PDFs with beta header `anthropic-beta: pdfs-2024-09-25`; max_tokens bumped to 8192 for large multi-week PDFs. (2) **Multi-week rotating menu support**: added `week_number` field to extraction schema; prompt now detects and extracts all weeks (1–4+); backend normalizes week_number 1–10; frontend: if maxWeek > 1 detected after extraction, shows new `week-select` step — card grid with week number + item count, user picks one week then proceeds to review; single-week menus skip this step and go straight to review. (3) **Format-aware prompt**: researched all real CACFP menu formats from NCA, ISBE, CT/NY state agencies, school districts — identified 4 distinct formats (A: section-row table; B: day-column table; C: narrative list; D: multi-week rotating cycle table); new prompt teaches AI all 4 formats with layout guide, Day-1-to-Monday mapping, component-label clues, and meal-type inference rules. Test files created: cacfp-format-a-section-rows.xlsx (NCA/ISBE style), cacfp-format-b-day-columns.xlsx (NY State style). Files: menusController.js (model, prompt, week_number normalization), MenuBuilderPage.jsx (ImportMenuModal: selectedWeek state, allWeeks state, loadWeekIntoReview(), week-select step JSX, updated header subtitle). | ✅ |
| 167 | Menu Builder — hide Sat/Sun columns when no weekend meals (2026-08-08 deferred) — "Saturday — No meals added" / "Sunday — No meals added" is confusing for 5-day programs; fix: only render day columns that have items OR are Mon–Fri by default; Sat/Sun shown only if items exist for those days | ⏳ |
| 170 | Daily Attendance tracking (2026-08-08) — `attendance_records` table (org_id, date, count, notes, UNIQUE org+date); attendanceController.js (listAttendance, upsertAttendance, deleteAttendance, compareAttendanceMeals — per-day comparison vs meal counts with anomaly flagging); routes/attendance.js mounted at /attendance; SiteMealCountPage.jsx: attendance card above meal tiles (−/+ input, Save button, saved badge, real-time anomaly warning "Lunch: 35 meals > 32 attendance"); claims: hasAttendance now queries real attendance_records (not totalCount alias); attendance restored to claims checklist as real check; `lunch_not_exceed_attendance` rule uses real attendance data. SQL: attendance_records.sql — run in Railway before deploying. | ✅ |
| 169 | Menu import — multi-week support (deferred 2026-08-08) — when a sponsor uploads a file with 2+ weeks of menus (e.g. a 4-week rotating cycle PDF or a CSV with WEEK 1 / WEEK 2 sections), the system should detect multiple weeks, show a week picker, and let the sponsor import each week independently. Currently the AI extraction tags items with week_number but the week picker is not triggering reliably — all items come back as week_number: 1 regardless of headers. Root cause: haiku doesn't reliably detect multi-week structure from CSV; sonnet should be tested for Excel/CSV too. Fix: (1) use sonnet for all extraction, not just PDFs; (2) test with real multi-week CACFP PDFs after Railway deploy; (3) ensure frontend maxWeek calculation handles NaN/Infinity edge cases gracefully. Priority: low — sponsors can import one week at a time for now. | ⏳ |
| 171 | Compliance + Documents merge (2026-08-08) — merged /documents route into /compliance; Documents nav item removed from sidebar; /documents route redirects to /compliance via React Navigate. ComplianceDrawer fully rewritten: loads real document records from GET /documents in parallel with compliance data; renders each required doc as a card (status pill, expiry date, per-doc Request + Upload buttons inline); missing docs show on red bg; expiring docs show "Upload new version" button; additional (non-required) docs listed below. Auto-open from URL: load() reads ?org=<uuid> param and opens matching org's drawer after data loads. Upload modal: clicking "Upload" on a specific doc type pre-selects it in the modal (prefilledDocType prop). Claims → Compliance deep link: CheckRow in ClaimsPage now accepts fixPath prop; Documents row shows purple "Fix →" link → /dashboard/sponsor/compliance?org=<siteId>. Backend fixes from same session: claimsController expiring_soon docs now counted as valid (status IN ('valid','pending_review','expiring_soon')); documentsController supersede logic now archives all active statuses not just 'requested'. Files: CompliancePage.jsx, SponsorDashboard.jsx (Navigate redirect + removed Documents nav), ClaimsPage.jsx (CheckRow + fixPath + Documents deep link) | ✅ |
| 172 | Document Intelligence — AI document validation (2026-08-09): Claude reads every uploaded PDF/image and classifies what it actually is vs what the sponsor selected. Three outcomes: 🟢 Verified (confidence ≥ 0.80, type matches → status = valid for sponsors), 🟡 Needs Review (confidence < 0.60 or can't read → status = pending_review), 🔴 Wrong Document (confident but wrong type → status = pending_review + warning shown). Architecture: `documentIntelligenceService.js` calls claude-3-5-sonnet with PDF beta support; DOC_TYPE_DESCRIPTIONS map gives Claude human-readable descriptions of each required doc type; backend decides outcome from Claude's structured JSON. Upload modal: after upload shows result inline — 🔴 wrong doc shows "Upload Correct File" button that resets file picker. Compliance drawer: each doc card now shows 🟢/🟡/🔴 badge next to status pill; wrong docs show red warning with reason text and trigger Request/Upload action buttons same as missing docs. DB: 4 new columns on documents table (verification_result, detected_type, confidence, verification_reason) — SQL: document_intelligence.sql (run in Railway ✅). Cloudflare R2 persistent storage wired in same session: AWS_ENDPOINT_URL + AWS_PUBLIC_URL env vars in Railway, storageService.js updated to support R2 endpoint + public URL + no ACL. Document view/delete: GET /documents/:id/file (auth-checked, redirects to R2 URL or serves local disk); DELETE /documents/:id (deletes from storage + DB); View + Delete buttons in ComplianceDrawer. Approve button: PATCH /documents/:id/status {status:'valid'} — handleApprove() in ComplianceDrawer; Approve button shown when doc.status='pending_review'. Files: programlink-backend/services/documentIntelligenceService.js (new), documentsController.js (classify after upload + serveDocument + deleteDocument), routes/documents.js (new GET/:id/file + DELETE/:id routes), storageService.js (R2 support), CompliancePage.jsx (INTEL_BADGE + IntelBadge + openDocumentFile + handleDelete + handleApprove + View/Delete/Approve buttons) | ✅ |
| 173 | Compliance polish (2026-08-12) — (1) Sponsor-created sites reach 100%: compliance.js now detects `sponsorCreated = !app_status && org_status='active'` and skips the -20 application penalty; tier logic also fixed so sponsor-created sites with missing docs don't get flagged 'overdue' for lack of application; `sponsor_created` boolean passed to frontend. (2) Application section hidden from drawer for sponsor-created orgs — only shown when site went through the application flow. (3) IntelBadge fix: "Needs Review" badge hidden when doc.status='valid' (no contradictory badge after sponsor approves). (4) Score bar hint: "Application pending" note only shown for non-sponsor-created orgs. Files: compliance.js, CompliancePage.jsx | ✅ |
| 174 | Document Intelligence — wrong document detection deferred (2026-08-12) — Upload Document modal runs Claude classification on every upload but confidence returns below 0.80 for generated/test PDFs so all docs fall into "Needs Review" instead of "Verified" or "Wrong Document". Real-world sponsor PDFs may trigger correctly. Fix when revisiting: (1) lower confidence threshold from 0.80 to 0.65 for "verified"; (2) improve DOC_TYPE_DESCRIPTIONS; (3) test with real scanned W-9, food permit, insurance cert. Currently: sponsors can Approve manually after viewing the PDF. | ⏳ |
| 175 | Claims doc type mismatch fix (2026-08-12) — claimsController.js was checking `stateConfig.requiredDocuments` (flat list including kitchen docs: w9, food_permit) against sites; sites only need enrollment/license/insurance/health_cert; added `REQUIRED_DOCS_BY_TYPE` constant in claimsController.js matching compliance.js exactly; sites now pass Documents check when all 4 site docs are uploaded. | ✅ |
| 177 | Individual Child Attendance Tracking (2026-08-12) — Per USDA 7 CFR 226.10(d): individual child names must be documented each day. New `child_attendance` table (org_id, child_id, date, is_present, had_breakfast, had_lunch, had_snack, had_supper, UNIQUE child_id+date). Two new backend endpoints: GET /attendance/roster (loads children + their day's record in one query via LEFT JOIN) and POST /attendance/roster (bulk upserts child_attendance, recalculates totals, auto-writes meal_counts + attendance_records for backward compat). Frontend: `src/pages/attendance/AttendancePage.jsx` — single component handles both roles: sponsor sees site picker dropdown, site user auto-loads own org; date nav with ◄ ► arrows; child roster table (Present/Absent toggle, per-meal checkboxes disabled when absent); Mark All Present / Absent bulk buttons; click meal column header to toggle all present children for that meal; live daily summary bar (auto-calculated as user clicks); Save → auto-posts meal counts. Wired into sponsor (Operations section) and site (Daily Ops section) sidebars with CalendarCheck icon. SQL: child_attendance.sql — run in Railway before deploying. | ✅ |
| 176 | Unified Daily Record — meal counts + attendance merged (2026-08-12) — SiteMealCountPage.jsx fully rewritten per Hashi's spec: (1) Single "Children Present" field at top = attendance record for the day; (2) Per-meal inputs (B/L/S/Su) below — each turns red and blocks submit if count exceeds children present; (3) One "Save Daily Record" button writes meal_counts table + attendance_records table simultaneously — no double entry; (4) 7-day identical pattern detection → soft amber warning (non-accusatory, allows save, prompts manual verification); (5) History shows `43 present · B:43 L:40 S:23` format; (6) Copy Yesterday copies both attendance and per-meal counts. File: src/pages/site/SiteMealCountPage.jsx | ✅ |
| 178 | Dashboard BlockingIssues fix (2026-08-13) — BlockingIssues component was showing "Jobsallday Hasn't Submitted Meal Counts" even when the claims engine reported 100% readiness. Root cause: BlockingIssues merges two independent data sources — `intel.issues` (claims engine, checks real per-month data) and `warnings` (/warnings endpoint, checks MAX(mc.date) and month-to-date totals with different logic). At 100% readiness the claims engine has no issues, but stale /warnings entries still fire. Fix: `const claimReady = (intel?.readinessScore ?? 0) >= 100; const MEAL_COUNT_WARNING_TYPES = ['missing_counts', 'no_counts_this_month']; const filteredWarnings = claimReady ? warnings.filter(w => !MEAL_COUNT_WARNING_TYPES.includes(w.type)) : warnings;` — suppresses meal-count warnings from the frontend when the authoritative claims engine says 100% ready. File: SponsorDashboard.jsx. Commit: ac7f717 | ✅ |
| 179 | State Rule Book data improvements (2026-08-13) — Updated STATE_RESOURCES in StateRuleBookPage.jsx for all 5 states: OH: agency renamed to "Ohio Dept. of Education and Workforce (DEW)" (ODE no longer exists), portalName "CRRS — Claims Reimbursement and Reporting System", portal URL updated to childnutrition.ode.state.oh.us, added OHID User Profile as required form, 4 new tips about CRRS/OHID/DEW rename; CA: agency "California Dept. of Education (CDE) — Child Nutrition", portalName "CNIPS — Child Nutrition Information and Payment System", phone 916-323-1135, email cacfp@cde.ca.gov, training deadline added, note about CDSS for family day care homes; TX: Added training deadline, clarified SquareMeals as TDA's online claims system; VA: Added email NutritionPrograms@doe.virginia.gov, Procurement Plan as required form, training/site review deadlines, tips about management problem flags; CO: Full office name "Office of Children, Youth & Families", training/site review deadlines added. Commit: 8e0a583 | ✅ |
| 180 | Demo pages sync (2026-08-13) — SponsorDemo.jsx and SiteDemo.jsx updated to reflect all recent product changes. SponsorDemo: removed "Delivery Plans" nav item (merged into Deliveries tabs per Task #160), removed "Documents" nav item (merged into Compliance per Task #171), added Attendance nav + SponsorAttendancePage (per-child roster with Present/Absent + meal checkboxes, auto-saves to meal counts), added Menu Cycles nav + MenuCyclesPage (cycle library cards, week assignments, apply-to-calendar), added State Rule Book nav + StateRuleBookDemoPage (TX rates/deadlines/agency, Federal Meal Pattern tab), rewrote DeliveriesPage with two tabs ("Today's Deliveries" interactive checklist + "Scheduled Routes" templates). SiteDemo: added Attendance nav + SiteAttendancePage (per-child roster), rewrote MealCountsPage as unified daily record (Children Present field at top, 4 meal types including Supper, blocks save if meal > attendance, one Save Daily Record button), added Present column to history table. Commit: f2b344c | ✅ |
| 186 | Connected Sites / Kitchen Directory removed (2026-08-26) — Hashi flagged he didn't understand what "Connected Sites" (kitchen nav) / "My Kitchen" (site nav) did. Investigation found it was a fully vestigial parallel system: a site-requests-connection-to-kitchen workflow (`kitchen_site_connections` table) that was never wired to anything real — deliveries, production schedules, and meal counts all derive the kitchen↔site relationship from `delivery_plans.kitchen_id`/`site_id` instead. Confirmed creating a delivery_plan never touches `kitchen_site_connections` and nothing reads from it except its own controller. Removed per Hashi's explicit call ("remove entirely, including unused backend code and navigation, do not replace"): nav item + route removed from KitchenDashboard.jsx and SiteDashboard.jsx; `src/pages/kitchen/KitchenDirectoryPage.jsx` and `src/hooks/useKitchenDirectory.js` deleted; backend `routes/kitchenDirectory.js` + `controllers/kitchenDirectoryController.js` deleted and unmounted from `routes/index.js`; sponsor's KitchensPage.jsx detail panel also had a "Sites served" stat + "Connected sites" list that read from the same dead `/kitchen-directory` endpoint — removed rather than left silently broken. `kitchen_site_connections` DB table left in place (unused, harmless) — not dropped. | ✅ |
| 185 | Kitchen Production Records rebuilt to match Sponsor's UI (2026-08-26) — Hashi's own kitchen-side testing found the kitchen's Production Records page was a completely different (older, weaker) design than the sponsor's kitchen-entry flow: a 7-day×4-meal grid vs. the sponsor's SmartProductionForm (menu auto-fill, enrollment prefill, planned-vs-enrolled compliance warning, previous-record sidebar with Copy Entire Meal/Copy Yesterday, print kitchen sheet, grouped record history with inline edit). Extracted the sponsor's implementation into `src/components/production/ProductionRecordShared.jsx` (SmartProductionForm, PreviousRecordPanel, RecordHistory + shared constants/helpers) so both pages render identically instead of drifting; also deleted a ~275-line unused `RecordForm` component that SponsorProductionRecordsPage.jsx still defined but never rendered since Task #163 replaced it. New `src/pages/kitchen/ProductionRecordsPage.jsx` fetches its own org via GET /auth/me and renders the shared form with no kitchen picker (a kitchen only ever logs for itself). Backend already scoped every endpoint correctly for role='kitchen' — no backend changes needed. Net -679 lines despite feature parity gain, from dedup. Commit 65969d6. | ✅ |
| 184 | Fixed `column mc.org_id does not exist` — three separate breaks from one wrong-column mistake (2026-08-26) — `meal_counts` has never had an `org_id` column, only `site_id` (who was fed) and `kitchen_id` (who prepared it). Found via Railway deploy logs after the new Kitchen Overview hero banner's production card 500'd for a kitchen ("donovan") with 2 fully correct, active delivery plans and today's instances already generated — DB was 100% right, the read was wrong. (1) `deliveryPlansController.js getKitchenProduction` — `LEFT JOIN meal_counts mc ON mc.org_id = s.id` → fixed to `mc.site_id = s.id`; this is what broke the kitchen's "My Deliveries" and Overview pages. (2) `scheduledJobs.js checkMealCountReminders` (daily 4pm UTC cron) — subquery selected `mc.org_id`, meaning the "you haven't submitted meal counts today" reminder has likely never fired for anyone since this cron was added. Fixed with a site_id/kitchen_id UNION. (3) `routes/auditLog.js GET /my-activity` — same wrong column inside a larger OR'd query, so this endpoint 500'd for every user on every call (currently unused — `ActivityTimeline.jsx` is imported but not rendered anywhere). All fixed and verified via `node --check`. Commit b8bfa88. | ✅ |
| 183 | Kitchen Overview: hero banner + per-meal-type checklist, parity with Site (2026-08-26) — Hashi compared Kitchen and Site overview screenshots side by side: Site had a brand-colored hero banner (greeting, delivery/meal-counts/documents status chips) and a granular per-meal checklist; Kitchen had a plain text header and one vague "Submit end-of-day counts" line. New `KitchenHeroBanner` component mirrors Site's `GoodMorningBanner` pattern exactly; `useKitchenData()` now also fetches `/auth/me` so the banner can greet the kitchen by its real org name. `KitchenDailyChecklist` rebuilt to list one item per meal type actually scheduled in today's production (Submit breakfast/lunch/snack/supper count, each tied to `todayCount[type] > 0`) instead of one opaque line, falling back to a generic item on days with no scheduled production. Commit 8b30376. Also same session: removed a dead "+ Add Schedule" header button on the sponsor's Deliveries page whose onClick (`setActiveTab('schedules')`) only ever ran while already on that tab — a pure no-op, fully redundant with the working "New Schedule" button already on screen (commit d1118d8). | ✅ |
| 181 | Outreach re-engagement batch (2026-08-13) — 26 personalized re-engagement emails written for all previously contacted sponsors across 5 states: Iowa (5 contacts — NEICAC, Polk County, MICA, Community Action SE Iowa, Iowa state office), Ohio (4 — Action for Children, Children's Hunger Alliance, CCRC, Ohio DEW state office), California (10 — all orgs from July 2026 outreach), Virginia (3 — ChildSavers/Hali Riley, Child Care Resources Inc/Donald Reese Goff, The Planning Council/Lisa Lyons), Colorado (4 — Kids' Nutrition Company/Deirdre Byerly, Southwest Food Program/Carrie Dyster, Kidcare Nutrition/Laura McCabe, Colorado Food Cluster/Kristen Collins). Key feature highlights used: per-child attendance tracking (USDA 7 CFR 226.10(d)), Claim Intelligence dollar amounts, 20-minute screen share ask. 4 Iowa/Ohio contacts flagged as needing email addresses. Saved to Desktop/outputs/reengagement-outreach-aug-2026.md. Strategy: committed to video calls/screen shares (no more avoiding calls). | ✅ |
